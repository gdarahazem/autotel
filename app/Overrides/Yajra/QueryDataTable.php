...
class QueryDataTable extends DataTableAbstract
{
    ...
    /**
     * Cache query with tags if not null and if is cached is true
     *
    ...
    public function cacheQuery(bool $isCached = false, \Illuminate\Cache\TaggedCache $cache = null)
    {
        $this->cache = $cache;
    ...
    {
        $sql = \App\Helpers\Helper::getSqlWithBindings($this->query);
        if ($this->isCached) {
            if ($this->cache != null) {
                return $this->cache->rememberForever($sql, function () {
                    return $this->query->get();
                });
            } else {
                return \Illuminate\Support\Facades\Cache::rememberForever($sql, function () {
                    return $this->query->get();
                });
            }
